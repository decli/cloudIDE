...
两条从 POC 里换来的硬规矩（见[验证结果](06-poc-findings.md) 3.1 和 3.5）：

- **验证配置不能放在 agent 能写的目录里。** 锁住 `pyproject.toml` 不够，agent 可以新建一个优先级更高的 `ruff.toml` 把它架空。检查应该由编排侧用平台自己的配置执行。
- **预览、验收测试、生产必须是同一种路径形态。** POC 里站点部署在子目录、验收测试却跑在根目录，结果绝对路径链接线上全部 404，测试却是全绿的。环境差一点，测试就失去意义。
- **验收标准锁定之前，必须先验证它自己跑得起来。** 锁定一份有错的规格，等于给 agent 发一个不可能完成的任务——它要么反复烧钱，要么被迫去绕过规则。POC 里就出过这事（3.6）。
