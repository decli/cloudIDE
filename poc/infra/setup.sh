#!/bin/sh
# 一键把本地 Gitea + Actions runner 拉起来，并把凭证写回 poc/.env。
# 幂等：重复执行不会破坏已有数据。
set -e
cd "$(dirname "$0")"
...
echo
echo "完成。"
echo "  Gitea:    http://localhost:3000   账号 $USERNAME / $PASSWORD"
echo "  runner:   docker compose -f infra/docker-compose.yml logs -f runner"
